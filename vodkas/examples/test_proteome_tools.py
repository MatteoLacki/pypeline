...
debug = True
rawdatapath = sys.argv[1].encode('unicode-escape').decode()#raw string
...
# apexOutPath, apex_proc = apex3d(rawdatapath, temp_folder,
#                                 write_binary=True,
#                                 capture_output=True,
#                                 debug=True)
# apexOutBIN = apexOutPath.with_suffix('.bin')

# pep3dOutPath, pep_proc = peptide3d(apexOutBIN, temp_folder,
#                                    write_binary=True,
#                                    min_LEMHPlus=350.0,
#                                    capture_output=True,
#                                    debug=True)
...
for parameters_file in (proj_folder/"params").iterdir():
    ...
    iadbsOutXML, iadbs_proc = iadbs(pep3dOutXML,
                                    ...
                                    debug=True)
    print(iadbs_proc)
    wx2csv(iadbsOutXML, temp_folder/parameters_file.stem/"report.csv", debug=debug)
